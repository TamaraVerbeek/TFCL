...
def main(file,recent_buffer_size,hard_buffer_size,MAS_weight):
    ...
    if os.path.isfile('storage/ROI.pkl'):
        with open('storage/ROI.pkl', 'rb') as f:
            d = pickle.load(f)
    else:
        d = Data(dataName,
        ...
        d.prepare(s)
        with open("storage/ROI.pkl", "wb") as f:
             pickle.dump(d, f)
